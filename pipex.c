...
/*   Updated: 2023/03/08 10:09:09 by rreis-de         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_children(int ac, char **av, char **env, t_child *children)
{
	...
	while (children)
	{
		...
		if (children == child_last(children))
			children->fd[1] = open(av[ac - 1], O_CREAT \
			| O_TRUNC | O_RDWR, 0644);
...
int	make_children(int ac, char **av, char **env)
{
	...
	while (i <= ac - 2)
	{
		child_add_back(&children, get_gpath(paths, \
		string_counter(path, ':'), av[i]), ft_split(av[i], ' '));
		i++;
	}
